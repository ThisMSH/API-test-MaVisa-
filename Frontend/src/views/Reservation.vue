    ...
    methods: {
        async getData() {
            if (this.token == '') {
                ...
                let response = await axios.get('http://localhost/MaVisa/Backend/api/updateData.php?token=' + this.token)
        ...
        remove() {
            try {
                axios.delete('http://localhost/MaVisa/Backend/api/delete.php?token=' + this.token)
                localStorage.clear()
